...
/// <summary>
/// Defines the <see cref="ExtractService" />.
/// </summary>
public class ExtractService
{
    ...
    public ExtractService(string sourceFile, bool replace = true, string @namespace = null)
    {
    ...
    public Dictionary<string, string> Extract(string resxFile)
    {
        ...
        if (nodes != null && nodes.Count > 0)
        {
            ...
            if (!Replace)
                extractedFile = extractedFile.Replace(
                    Path.GetExtension(extractedFile),
                    $".auto{Path.GetExtension(extractedFile)}"
                );
    ...
    public string GetReferenceName(string resxFile, string key)
    {
        ...
        var className =
            Path.GetFileNameWithoutExtension(resxFile) + (Replace ? string.Empty : "_auto");
